...
void RTLSDRInput::webapiFormatDeviceSettings(SWGRPX100::SWGDeviceSettings& response, const RTLSDRSettings& settings)
{
...
{
    (void) errorMessage;
    response.setRtlSdrReport(new SWGRPX100::SWGRtlSdrReport());
    response.getRtlSdrReport()->init();
    webapiFormatDeviceReport(response);
    return 200;
}

void RTLSDRInput::webapiFormatDeviceReport(SWGRPX100::SWGDeviceReport& response)
{
    response.getRtlSdrReport()->setGains(new QList<SWGRPX100::SWGGain*>);

    for (std::vector<int>::const_iterator it = getGains().begin(); it != getGains().end(); ++it)
    {
        response.getRtlSdrReport()->getGains()->append(new SWGRPX100::SWGGain);
...
void RTLSDRInput::webapiReverseSendSettings(QList<QString>& deviceSettingsKeys, const RTLSDRSettings& settings, bool force)
{
    ...
    swgDeviceSettings->setRtlSdrSettings(new SWGRPX100::SWGRtlSdrSettings());
    SWGRPX100::SWGRtlSdrSettings *swgRtlSdrSettings = swgDeviceSettings->getRtlSdrSettings();
