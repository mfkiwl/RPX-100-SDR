    ...
    m_initialBytesCount(0)
{
    ...
    m_metaRecord = new sigmf::SigMF<sigmf::Global<core::DescrT, RPX100::DescrT>,
    ...
    if (!m_recordOn)
    {
        ...
        if (QFile::exists(fileName + ".sigmf-data") && QFile::exists(fileName + ".sigmf-meta"))
        {
            ...
            try
            {
                ...
                metaStream.close();
                std::string RPX100Version = m_metaRecord->global.access<RPX100::GlobalT>().version;

                if (RPX100Version.size() != 0)
                {
                    ...
                    for (auto capture : m_metaRecord->captures)
                    {
                        ...
                        int32_t sampleRate = capture.get<RPX100::DescrT>().sample_rate;
                ...
                {
                    qDebug("SigMFFileRecord::setFileName: SigMF not recorded with RPX100. Recreating files...");
                    m_initialBytesCount = 0;
                    m_initialMsCount = 0;
                    m_recordStart = true;
                }
...
{
    m_metaRecord->global.access<core::GlobalT>().author = "RPX100";
    m_metaRecord->global.access<core::GlobalT>().description = "RPX100 SigMF I/Q recording file";
    ...
    m_metaRecord->global.access<RPX100::GlobalT>().version = QString(QCoreApplication::applicationVersion()).toStdString();
    m_metaRecord->global.access<RPX100::GlobalT>().qt_version = QT_VERSION_STR;
    m_metaRecord->global.access<RPX100::GlobalT>().rx_bits = SDR_RX_SAMP_SZ;
    m_metaRecord->global.access<RPX100::GlobalT>().arch = QString(QSysInfo::currentCpuArchitecture()).toStdString();
    m_metaRecord->global.access<RPX100::GlobalT>().os = QString(QSysInfo::prettyProductName()).toStdString();
...
void SigMFFileRecord::makeCapture()
{
    if (m_sampleCount)
    {
        ...
        // Add new capture
        auto recording_capture = sigmf::Capture<core::DescrT, RPX100::DescrT>();
        ...
        recording_capture.get<RPX100::DescrT>().sample_rate = m_sampleRate;
        recording_capture.get<RPX100::DescrT>().tsms = m_captureStartDT.toMSecsSinceEpoch();
