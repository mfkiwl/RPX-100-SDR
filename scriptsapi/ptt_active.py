...
''' Active PTT
  Handles the switchover between two arbitrary device sets
    ...
    - It can pilot a LimeRFE device via USB through RPX100 API (by giving TTY device with --limerfe-dev parameter)
...
# ======================================================================
def start_device(device_index, RPX100_ip, RPX100_port):
    ...
    if r.status_code // 100 == 2:
        ...
        if state is not None:
            if state == "idle":
                ...
                if LIMERFE_DEVICE:
                    limerfe_switch(device_index, True, RPX100_ip, RPX100_port)
            else:
...
# ======================================================================
def stop_device(device_index, RPX100_ip, RPX100_port):
    ...
    if r.status_code // 100 == 2:
        ...
        if state is not None:
            if state == "running":
                ...
                if LIMERFE_DEVICE:
                    limerfe_switch(device_index, False, RPX100_ip, RPX100_port)
            else:
...
# ======================================================================
def set_focus(device_index, RPX100_ip, RPX100_port):
...
# ======================================================================
def set_center_frequency(new_frequency, device_index, RPX100_ip, RPX100_port):
...
# ======================================================================
def limerfe_switch(originator_index, start, RPX100_ip, RPX100_port):
    ...
    """
# ----------------------------------------------------------------------
    endpoint_url = f'http://{RPX100_ip}:{RPX100_port}/RPX100/limerfe/run'
    try:
...
# ======================================================================
@app.route('/RPX100/deviceset/<int:deviceset_index>/device/run', methods=['GET', 'POST', 'DELETE'])
...
# ----------------------------------------------------------------------
    ...
    if originator_index is None or direction is None:
        print('device_run: RPX100 reverse API v4.5.2 or higher required. No or invalid originator information')
        return ""
    ...
    if request.method == 'POST':
        print(f'device_run: Device {originator_index} (direction={direction}) has started at {RPX100_ip}:{RPX100_API_PORT}')
        if originator_index not in RUNNING:
            RUNNING.add(originator_index)
        if other_device_index in RUNNING:
            time.sleep(DELAY)
            stop_device(other_device_index, RPX100_ip, RPX100_API_PORT)
        ...
        print(f'device_run: Device {originator_index} (direction={direction}) has stopped at {RPX100_ip}:{RPX100_API_PORT}')
        if originator_index in RUNNING:
            RUNNING.remove(originator_index)
        if other_device_index not in RUNNING:
            time.sleep(DELAY)
            start_device(other_device_index, RPX100_ip, RPX100_API_PORT)
            set_focus(other_device_index, RPX100_ip, RPX100_API_PORT)
...
# ======================================================================
@app.route('/RPX100/deviceset/<int:deviceset_index>/device/settings', methods=['GET', 'PATCH', 'PUT'])
...
# ----------------------------------------------------------------------
    ...
    if new_frequency and FREQ_SYNC:
        ...
        if FREQ_HAS_CHANGED:
            ...
            set_center_frequency(new_frequency, other_device_index, RPX100_ip, RPX100_API_PORT)
...
# ----------------------------------------------------------------------
    ...
    parser.add_argument("-p", "--port-sdr", dest="RPX100_port", help="RPX100 REST API port", metavar="PORT", type=int)
    ...
    return options.addr, options.port, options.RPX100_port, options.delay, options.freq_sync, other_dict, options.limerfe_dev
...
# ----------------------------------------------------------------------
    ...
    addr, port, RPX100_API_PORT, DELAY, FREQ_SYNC, OTHER_DICT, LIMERFE_DEVICE = getInputOptions()
    print(f'main: starting: RPX100 port: {RPX100_API_PORT} links: {OTHER_DICT} freq sync: {FREQ_SYNC} LimeRFE: {LIMERFE_DEVICE}')
