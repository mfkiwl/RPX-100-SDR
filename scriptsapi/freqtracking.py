...
Listens on a TCP port for rpx-100 reverse API requests.
...
In the rpx-100 instance you must activate the reverse API of the FreqTracker channel and the controlled channel(s)
...
rpx-100_API_ADDR = None
rpx-100_API_PORT = 8091
TRACKER_OFFSET = 0
TRACKER_DEVICE = 0
TRACKING_DICT = {}
...
# ----------------------------------------------------------------------
    parser = argparse.ArgumentParser(description="Manages PTT from an rpx-100 instance automatically")
    ...
    parser.add_argument("-a", "--address-sdr", dest="rpx-100_address", help="rpx-100 REST API address (defaults to calling address)", metavar="ADDRESS", type=str)
    parser.add_argument("-p", "--port-sdr", dest="rpx-100_port", help="rpx-100 REST API port (default 8091)", metavar="PORT", type=int)
    ...
    if options.port == None:
        options.port = 8888
    if options.rpx-100_port == None:
        options.rpx-100_port = 8091

    return options.addr, options.port, options.rpx-100_address, options.rpx-100_port, options.tracker_frequency, options.transverter_device, options.refcorr_limit

# ======================================================================
def get_rpx-100_ip(request):
    """ Extract originator address from request """
# ----------------------------------------------------------------------
    if rpx-100_API_ADDR is not None:
        return rpx-100_API_ADDR
...
# ======================================================================
def get_device_frequency(rpx-100_ip, rpx-100_port, device_index):
    """ Obtain the device center frequency from either the settings or
        the report
    """
# ----------------------------------------------------------------------
    base_url = f'http://{rpx-100_ip}:{rpx-100_port}/rpx-100'
...
# ======================================================================
def adjust_channels(rpx-100_ip, rpx-100_port):
...
# ----------------------------------------------------------------------
    ...
    for k in remove_keys:
        ...
        if tracking_item:
            ...
            device_index = k[0]
            channel_index = k[1]
            print(f'rpx-100: {rpx-100_ip}:{rpx-100_port} Removed {channel_type} [{device_index}:{channel_index}]')

# ======================================================================
def adjust_xvtr(rpx-100_ip, rpx-100_port, tracker_device_index, tracker_channel_index, tracker_content, tracker_offset):
...
# ----------------------------------------------------------------------
    ...
    base_url = f'http://{rpx-100_ip}:{rpx-100_port}/rpx-100'
    correction = 0
    tracker_device_frequency = get_device_frequency(rpx-100_ip, rpx-100_port, tracker_device_index)
    # get correction from report
    if tracker_device_frequency is None:
        print(f'rpx-100::adjust_xvtr: {rpx-100_ip}:{rpx-100_API_PORT} get tracker device {tracker_device_index} frequency failed')
        return
    ...
    if r.status_code // 100 != 2:
        print(f'rpx-100::adjust_xvtr: {rpx-100_ip}:{rpx-100_API_PORT} get transverter device {XVTR_DEVICE} settings failed')
        return
    device_content = r.json()
    for xvtr_freq in gen_dict_extract('transverterDeltaFrequency', device_content):
        ...
        if r.status_code // 100 != 2:
            print(f'rpx-100::adjust_xvtr: {rpx-100_ip}:{rpx-100_API_PORT} transverter device {XVTR_DEVICE} adjust failed')
            return
        # tracker
        ...
        if r.status_code // 100 != 2:
            print(f'rpx-100::adjust_xvtr: {rpx-100_ip}:{rpx-100_API_PORT} tracker [{tracker_device_index}:{tracker_channel_index}] adjust failed')
...
# ----------------------------------------------------------------------
    ...
    rpx-100_ip = get_rpx-100_ip(request)
...
# ----------------------------------------------------------------------
    ...
    if orig_device_index is None or orig_channel_index is None:
        print('device_settings: rpx-100 reverse API v4.5.2 or higher required. No or invalid originator information')
        return "rpx-100 reverse API v4.5.2 or higher required "
    rpx-100_ip = get_rpx-100_ip(request)
    ...
    for freq_offset in gen_dict_extract('inputFrequencyOffset', content):
        if channel_type == "FreqTracker":
            print(f'rpx-100: {rpx-100_ip}:{rpx-100_API_PORT} Tracker [{orig_device_index}:{orig_channel_index}] at {freq_offset} Hz')
            TRACKER_OFFSET = freq_offset
            TRACKER_DEVICE = orig_device_index
            adjust_channels(rpx-100_ip, rpx-100_API_PORT)
            if TRACKER_FREQUENCY is not None and XVTR_DEVICE is not None: # optionally lock tracker to the beacon it is supposed to follow
                adjust_xvtr(rpx-100_ip, rpx-100_API_PORT, orig_device_index, orig_channel_index, content, freq_offset)
        else:
            register_channel(orig_device_index, orig_channel_index, freq_offset, content)
            print(f'rpx-100: {rpx-100_ip}:{rpx-100_API_PORT} {channel_type} [{orig_device_index}:{orig_channel_index}] at {freq_offset} Hz')
    return "OK processed "
...
# ----------------------------------------------------------------------
    global rpx-100_API_ADDR
    global rpx-100_API_PORT
    global TRACKER_FREQUENCY
    global XVTR_DEVICE
    global REFCORR_LIMIT
    addr, port, rpx-100_API_ADDR, rpx-100_API_PORT, TRACKER_FREQUENCY, XVTR_DEVICE, refcorr_limit = getInputOptions()
