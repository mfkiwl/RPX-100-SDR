...
Listens on a TCP port for RPX100 reverse API requests.
...
In the RPX100 instance you must activate the reverse API of the FreqTracker channel and the controlled channel(s)
...
RPX100_API_ADDR = None
RPX100_API_PORT = 8091
TRACKER_OFFSET = 0
TRACKER_DEVICE = 0
TRACKING_DICT = {}
...
# ----------------------------------------------------------------------
    parser = argparse.ArgumentParser(description="Manages PTT from an RPX100 instance automatically")
    ...
    parser.add_argument("-a", "--address-sdr", dest="RPX100_address", help="RPX100 REST API address (defaults to calling address)", metavar="ADDRESS", type=str)
    parser.add_argument("-p", "--port-sdr", dest="RPX100_port", help="RPX100 REST API port (default 8091)", metavar="PORT", type=int)
    ...
    if options.port == None:
        options.port = 8888
    if options.RPX100_port == None:
        options.RPX100_port = 8091

    return options.addr, options.port, options.RPX100_address, options.RPX100_port, options.tracker_frequency, options.transverter_device, options.refcorr_limit

# ======================================================================
def get_RPX100_ip(request):
    """ Extract originator address from request """
# ----------------------------------------------------------------------
    if RPX100_API_ADDR is not None:
        return RPX100_API_ADDR
...
# ======================================================================
def get_device_frequency(RPX100_ip, RPX100_port, device_index):
    """ Obtain the device center frequency from either the settings or
        the report
    """
# ----------------------------------------------------------------------
    base_url = f'http://{RPX100_ip}:{RPX100_port}/RPX100'
...
# ======================================================================
def adjust_channels(RPX100_ip, RPX100_port):
...
# ----------------------------------------------------------------------
    ...
    for k in remove_keys:
        ...
        if tracking_item:
            ...
            device_index = k[0]
            channel_index = k[1]
            print(f'RPX100: {RPX100_ip}:{RPX100_port} Removed {channel_type} [{device_index}:{channel_index}]')

# ======================================================================
def adjust_xvtr(RPX100_ip, RPX100_port, tracker_device_index, tracker_channel_index, tracker_content, tracker_offset):
...
# ----------------------------------------------------------------------
    ...
    base_url = f'http://{RPX100_ip}:{RPX100_port}/RPX100'
    correction = 0
    tracker_device_frequency = get_device_frequency(RPX100_ip, RPX100_port, tracker_device_index)
    # get correction from report
    if tracker_device_frequency is None:
        print(f'RPX100::adjust_xvtr: {RPX100_ip}:{RPX100_API_PORT} get tracker device {tracker_device_index} frequency failed')
        return
    ...
    if r.status_code // 100 != 2:
        print(f'RPX100::adjust_xvtr: {RPX100_ip}:{RPX100_API_PORT} get transverter device {XVTR_DEVICE} settings failed')
        return
    device_content = r.json()
    for xvtr_freq in gen_dict_extract('transverterDeltaFrequency', device_content):
        ...
        if r.status_code // 100 != 2:
            print(f'RPX100::adjust_xvtr: {RPX100_ip}:{RPX100_API_PORT} transverter device {XVTR_DEVICE} adjust failed')
            return
        # tracker
        ...
        if r.status_code // 100 != 2:
            print(f'RPX100::adjust_xvtr: {RPX100_ip}:{RPX100_API_PORT} tracker [{tracker_device_index}:{tracker_channel_index}] adjust failed')
...
# ----------------------------------------------------------------------
    ...
    RPX100_ip = get_RPX100_ip(request)
...
# ----------------------------------------------------------------------
    RPX100_ip = get_RPX100_ip(request)
    print(f'RPX100 IP: {RPX100_ip}')
    return 'Hello, RPX100!'


# ======================================================================
@app.route('/RPX100/deviceset/<int:deviceset_index>/channel/<int:channel_index>/settings', methods=['GET', 'PATCH', 'PUT'])
...
# ----------------------------------------------------------------------
    ...
    if orig_device_index is None or orig_channel_index is None:
        print('device_settings: RPX100 reverse API v4.5.2 or higher required. No or invalid originator information')
        return "RPX100 reverse API v4.5.2 or higher required "
    RPX100_ip = get_RPX100_ip(request)
    ...
    for freq_offset in gen_dict_extract('inputFrequencyOffset', content):
        if channel_type == "FreqTracker":
            print(f'RPX100: {RPX100_ip}:{RPX100_API_PORT} Tracker [{orig_device_index}:{orig_channel_index}] at {freq_offset} Hz')
            TRACKER_OFFSET = freq_offset
            TRACKER_DEVICE = orig_device_index
            adjust_channels(RPX100_ip, RPX100_API_PORT)
            if TRACKER_FREQUENCY is not None and XVTR_DEVICE is not None: # optionally lock tracker to the beacon it is supposed to follow
                adjust_xvtr(RPX100_ip, RPX100_API_PORT, orig_device_index, orig_channel_index, content, freq_offset)
        else:
            register_channel(orig_device_index, orig_channel_index, freq_offset, content)
            print(f'RPX100: {RPX100_ip}:{RPX100_API_PORT} {channel_type} [{orig_device_index}:{orig_channel_index}] at {freq_offset} Hz')
    return "OK processed "
...
# ----------------------------------------------------------------------
    global RPX100_API_ADDR
    global RPX100_API_PORT
    global TRACKER_FREQUENCY
    global XVTR_DEVICE
    global REFCORR_LIMIT
    addr, port, RPX100_API_ADDR, RPX100_API_PORT, TRACKER_FREQUENCY, XVTR_DEVICE, refcorr_limit = getInputOptions()
