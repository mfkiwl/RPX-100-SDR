...
show_help() {
  ...
  Run rpx-100 client in a Docker container.
  -i name    Docker image name (default rpx-100/swagger/server)
  -t tag     Docker image tag version (default latest)
  -c name    Docker container name (default rpx-100swg_server)
...
image_tag="latest"
container_name="rpx-100swg_server"
...
docker run -it --rm \
    --name ${container_name} \
    ${http_port} \
    -v="/opt/build/rpx-100:/opt/build/rpx-100:rw" \
